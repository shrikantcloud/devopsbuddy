...
public class S3Service {
    ...
    public String storeProfileImage(MultipartFile uploadedFile, String username) {
        ...
        try {
            if (uploadedFile != null && !uploadedFile.isEmpty()) {
                ...
                File tmpProfileImageFile = new File(tmpImageStoredFolder.getAbsolutePath() + File.separatorChar + PROFILE_PICTURE_FILE_NAME
    ...
    private String storeProfileImageToS3(File resource, String username) {
        ...
        if (!resource.exists()) {
            ...
            throw new S3Exception(" The file " + resource.getAbsolutePath() + " doesnot exists!");
        }

        String rootBucketUrl = this.ensureBucketExists(bucketName);

        if (null == rootBucketUrl) {
            LOG.error("The bucket {} doesnot exists and the application was not able to create it. Hence, image wont be stored with profile",
                      rootBucketUrl);
        } else {
            ...
            try {
                ...
                LOG.error("A client exception occured while trying to store the profile image {} on s3." + "Profile image wont be stored",
